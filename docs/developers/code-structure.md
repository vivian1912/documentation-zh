...
* `api/` - java-tron节点对外提供的gRPC接口及数据结构
* `core/` - 节点间及节点内部各个模块间通信的数据结构
...
* `core/` - chainbase模块核心代码
    ...
    * `db/` 和 `db2/` 
        ...
        实现了可回退数据库。`db/` 目录提供核心接口与基类（如 `RevokingDatabase`、`TronStoreWithRevoking`），而实际的回滚实现是位于 `db2/` 目录下的 `SnapshotManager`。`SnapshotManager` 提供稳定的数据回滚机制，并支持底层数据库的扩展，是 java-tron 用于回退数据库的实现，其中的几个重要接口及实现类如下：
...
> chainbase 模块的详细解析——交易处理、状态回滚、区块持久化与数据持久化——见 [ChainBase 深入解析](chainbase.md)。
...
framework是 java-tron 的核心模块，也是节点的入口，framework 模块负责各个模块的初始化及业务逻辑的跳转，framework包含了节点对外提供的服务、区块广播及处理流程；底层的节点发现与连接传输由外部依赖 libp2p 提供，而 TRON 协议层（握手、保活、消息分发、同步与广播）仍由 framework 的 `core/net` 负责。
...
* `core/services` - 定义了对外提供的服务，其子目录`http/`包含了所有的http接口处理类，`jsonrpc/` 包含了所有的json-rpc接口处理类
* `core/net` - 消息处理及网络业务逻辑，其子目录`service/` 为交易及区块广播、区块抓取及同步逻辑
* `core/db/Manager.java` - 交易及区块校验并处理逻辑

> **说明**：早期版本中，节点发现与节点连接管理的底层实现位于 framework 模块的 `common/overlay` 目录（`discover`、`server` 等子目录）。该 P2P 网络底层现已抽离为独立的外部依赖 [`io.github.tronprotocol:libp2p`](https://github.com/tronprotocol/libp2p)，framework 中不再保留 `common/overlay` 与 `common/net` 目录。底层的节点发现与连接传输现由 libp2p 库提供，framework 通过 `core/net`（如 `TronNetService`）与之对接；其上的 TRON 协议层——握手、保活、消息分发、同步、广播以及 peer 业务状态管理——仍保留在 `core/net` 中。

> 其上的网络层位于 `core/net`。P2P 网络子系统的详细解析——区块同步、区块和交易广播——见 [P2P 网络深入解析](network.md)。
