...
> USDT 命令（`GetUSDTBalance`、`TransferUSDT`、`GetUsdtTransferById`）只在**主网、Nile 和
> Shasta** 网络上运行。在其他网络上它们会提示
> `This command does not support the current network.`
...
- [transfer-trc10](transfer-trc10.md)——TRC10 原生 token
- [contract](contract.md)——通用智能合约调用
