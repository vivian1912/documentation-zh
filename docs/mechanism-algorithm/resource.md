...
用户根据其为获取带宽而质押的 TRX 数量，按比例分享一个全网固定的总带宽池。
    ```
    额度 = (为获取带宽质押的 TRX / 整个网络为获取带宽质押的 TRX 总额) * (Bandwidth 上界) 
    ```
...
能量 (Energy) 是在 TRON 虚拟机（TVM）中执行智能合约指令时所需消耗的计算资源的计量单位。
...
```
您的能量额度 = (为获取 Energy 质押的 TRX / 整个网络为获取 Energy 质押的 TRX 总额) * 能量上界
```
**能量上界**是一个由委员会设定的网络参数（[#19](https://tronscan.io/#/sr/committee)），当前值为 180,000,000,000，可通过 TIP 修改。
